...
@api_view(['GET'])
def health(request):
    return Response({'status': 'ok', 'service': 'daily-organizer-api'})
...
def unified_search(request):
    ...
    if not query:
        return Response({'query': '', 'results': {'tasks': [], 'notes': [], 'goals': [], 'concept_links': [], 'mind_maps': []}})
    ...
    concept_results = ConceptLink.objects.filter(
        Q(source__icontains=query) | Q(target__icontains=query) | Q(relation__icontains=query)
    )[:10]
    mindmap_results = MindMap.objects.filter(title__icontains=query)[:10]
    ...
    results = {
        ...
        'concept_links': ConceptLinkSerializer(concept_results, many=True).data,
        'mind_maps': MindMapSerializer(mindmap_results, many=True).data,
    }
